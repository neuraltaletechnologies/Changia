...
export function generateStaticParams() {
  return getBlogEntries('fr').map((post) => ({ id: post.slug }));
}

function findPost(id: string) {
  return getBlogEntries('fr').find((p) => p.slug === id);
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return {
    ...
    description: post.data.contents[0] || `Lisez ${post.data.title} sur le blog de ScrewFast`,
    openGraph: {
...
export default async function FrenchBlogPostPage({ params }: Params) {
  const { id } = await params;
  const post = findPost(id);
  if (!post) notFound();
  const data = post.data;
  const relatedPosts = getBlogEntries('fr').filter((p) => p.id !== post.id);
  ...
  return (
    <>
      <section className="mx-auto max-w-3xl px-4 pt-6 pb-12 sm:px-6 lg:px-8 lg:pt-10">
        ...
        <PostFeedback title="Cet article était-il utile?" firstChoice="Oui" secondChoice="Non" />
      </section>
      ...
      <section className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="grid grid-cols-2 gap-6">
          {relatedPosts.map((entry) => (
            <CardRelated key={entry.id} blogEntry={entry} recentBlogLocale="fr" />
          ))}
        </div>
      </section>
    </>
