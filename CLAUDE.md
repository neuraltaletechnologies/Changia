...
- Four roles, three of them **platform-level** (no `organization_id`): `SUPER_ADMIN` (platform config, users, audit, marks payouts paid), `REVIEWER` (stage-1 "first review" on every org's campaigns), `ORG_ADMIN` (stage-2 "final approval" on every org's campaigns **and** payouts, plus platform-wide reports — but NOT donors/pools/reminders/users). Only `CAMPAIGN_MANAGER` is org-scoped by `organization_id` — it's the role created at registration, placed under the org it registers, and it runs assigned campaigns, manages donors/pools, and requests payouts/closures. `isPlatformRole()` in `modules/campaign/service.js` is the scoping gate. Campaigns a platform-level role creates fall under the lazily-created "Changia Platform" org.
