...
export function generateStaticParams() {
  return getInsightEntries('fr').map((post) => ({ id: post.slug }));
}

function findInsight(id: string) {
  return getInsightEntries('fr').find((p) => p.slug === id);
}

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const { id } = await params;
  const post = findInsight(id);
  if (!post) return { title: 'Perspectives' };
  return {
...
export default async function FrenchInsightPostPage({ params }: Params) {
  const { id } = await params;
  const post = findInsight(id);
  if (!post) notFound();
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:gap-12">
          <div>
            ...
            <div id="pin" className="mt-10 hidden space-y-4 md:block">
              ...
              <p className="text-sm text-pretty text-neutral-500">Table des matières :</p>
              <div id="toc">
