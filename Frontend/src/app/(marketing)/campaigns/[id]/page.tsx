...
export default async function CampaignDetailPage({ params }: Params) {
  ...
  return (
    <>
      <section className="mx-auto flex max-w-[85rem] flex-col px-4 py-10 sm:px-6 lg:px-8 lg:py-14 2xl:max-w-full">
        <div>
          <p className="mb-8 max-w-prose font-light text-pretty text-neutral-700 sm:text-xl dark:text-neutral-300">
            {campaign.category || 'Community'} campaign by {campaign.organizationName || 'a Changia organization'}
          </p>
        </div>
        <div className="flex flex-col items-center justify-between space-y-4 sm:flex-row sm:space-y-0">
          <div>
            ...
            <p className="text-lg text-neutral-600 dark:text-neutral-400">
              {formatTZS(campaign.raisedAmount)} raised of {formatTZS(campaign.publicTarget)} target ·{' '}
      ...
      <PublicCampaignTabs campaign={campaign} />
    </>
  );
}
