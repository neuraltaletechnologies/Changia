...
export function generateStaticParams() {
  return getBlogEntries('en').map((post) => ({ id: post.slug }));
}

function findPost(id: string) {
  return getBlogEntries('en').find((p) => p.slug === id);
}
...
export default async function BlogPostPage({ params }: Params) {
  ...
  const data: BlogData = post.data;
  const relatedPosts = getBlogEntries('en').filter((p) => p.id !== post.id);
      ...
      <section className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div className="grid grid-cols-2 gap-6">
          {relatedPosts.map((entry) => (
            <CardRelated key={entry.id} blogEntry={entry} />
          ))}
        </div>
      </section>
    </>
