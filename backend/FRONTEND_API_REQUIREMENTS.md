...
- **`PATCH /campaigns/:id/gifts/:giftId/status`** — same roles. JSON `{ status: "PLEDGED"|"SCHEDULED"|"RECEIVED"|"CANCELLED" }`. The manager advances a public pledge as they arrange the pickup / take delivery. Stages only move **forward** (`PLEDGED → SCHEDULED → RECEIVED`); `CANCELLED` is allowed from `PLEDGED`/`SCHEDULED` only, and `RECEIVED`/`CANCELLED` are final — a backward move returns `400 GIFT_STAGE_BACKWARD`. Only a `RECEIVED` gift counts toward the campaign's in-kind total. Returns the updated `Gift[]`.
