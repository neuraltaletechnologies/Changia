...
export function generateStaticParams() {
  return getInsightEntries('sw').map((post) => ({ id: post.slug }));
}

function findInsight(id: string) {
  return getInsightEntries('sw').find((p) => p.slug === id);
}

export async function generateMetadata({ params }: Params): Promise<Metadata> {
  const { id } = await params;
  const post = findInsight(id);
  if (!post) return { title: 'Uchambuzi' };
  return {
    title: post.data.title,
    description: post.data.description,
    openGraph: {
      title: `${post.data.title} | Uchambuzi | Changia`,
      description: post.data.description,
    },
  };
}

export default async function SwahiliInsightPostPage({ params }: Params) {
  const { id } = await params;
  const post = findInsight(id);
  if (!post) notFound();
      ...
      <div className="mx-auto max-w-(--breakpoint-xl) px-4 md:px-8">
        <div className="grid gap-8 md:grid-cols-2 lg:gap-12">
          <div>
            ...
            <div
              id="progress-mobile"
              className="fixed top-0 left-0 h-2 w-full bg-linear-to-r from-blue-400/30 to-blue-600 md:hidden"
            />
            <div id="pin" className="mt-10 hidden space-y-4 md:block">
              <div className="h-px w-full overflow-hidden bg-neutral-300 dark:bg-neutral-700">
                <div
                  id="progress"
                  className="h-px w-0 bg-linear-to-r from-blue-400/30 to-blue-600"
                />
              </div>
              <p className="text-sm text-pretty text-neutral-500">Jedwali la yaliyomo:</p>
              <div id="toc">
