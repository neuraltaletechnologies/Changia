...
- Four roles, three of them **platform-level** (no `organization_id`): `SUPER_ADMIN` (platform config, users, audit), `REVIEWER` (stage-1 "first review" on every org's campaigns), `ORG_ADMIN` (stage-2 "final approval" on every org's campaigns **and** payouts, plus platform-wide reports — but NOT donors/pools/reminders/users). Only `CAMPAIGN_MANAGER` is org-scoped by `organization_id` — it's the role created at registration, placed under the org it registers, and it runs assigned campaigns, manages donors/pools, requests payouts/closures, and **confirms the release of its own approved payout** (which atomically fires the mobile-money transfer — there is no separate super-admin "mark paid" step). `isPlatformRole()` in `modules/campaign/service.js` is the scoping gate. Campaigns a platform-level role creates fall under the lazily-created "Changia Platform" org.
- Payout lifecycle (`modules/payout/`): `REQUESTED → REVIEWED → APPROVED` (on hold) `→ PAID`. The mobile-money destination is captured with the request; one in-flight payout per campaign; the manager's `POST /payouts/:id/confirm` runs the ClickPesa payout inside a `db.withTransaction` row lock so a double-confirm can't double-pay.
